...
import math, random, hashlib, tempfile, base64
import gradio as gr
...
def to_morse(s):
    ...
    for w in s.lower().split():
        letters=[MORSE[ch] for ch in w if ch in MORSE]
...
def star_count_from_density(d): 
    return 250 + clamp(int(d),1,5)*80

def porosity_to_variance(p): 
    return 0.25 + (clamp(int(p),1,5)-1)*0.18
...
def morse_ring(cx,cy,R,message):
    ...
    unit=0.012*math.pi; cur_a=-math.pi/2; dot_r=3.2; dash_arc=unit*2.2
    for letters in words:
        for code in letters:
            for symbol in code:
                if symbol=='.':
                    x,y=polar_to_xy(cx,cy,R,cur_a); out.append(svg_circle(x,y,dot_r))
                    cur_a += unit*2.0
                elif symbol=='-':
...
def make_svg(name, hair_type, density, porosity, length, elasticity, message):
    ...
    if fam=="coil":   svg.append(pattern_coil(cx,cy,R,rnd,tight,var,arms=3,scale=length_to_scale(length)))
    elif fam=="curl": svg.append(pattern_curl(cx,cy,R,rnd,amp,freq,var,loops=6))
    elif fam=="wave": svg.append(pattern_wave(cx,cy,R,rnd,amp,freq,var))
    else:             svg.append(pattern_locs(cx,cy,R,rnd,amp,freq,var,strands=5))
...
def generate(name, hair_type, density, porosity, length, elasticity, message):
    ...
    svg = make_svg(name, hair_type, int(density), int(porosity), length, int(elasticity), message)
    b64 = base64.b64encode(svg.encode("utf-8")).decode("ascii")
    html_preview = f'<img alt="Star map preview" width="540" height="540" src="data:image/svg+xml;base64,{b64}"/>'
    tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".svg")
    tmp.write(svg.encode("utf-8")); tmp.close()
    return html_preview, tmp.name
...
# IMPORTANT: Do NOT call demo.launch() on Hugging Face Spaces
